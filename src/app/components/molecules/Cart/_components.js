...
const ShoppingButton = () => (
  <Link to="/">
    <Button.INFO theme={ButtonTheme.ROUNDED} size={isMobile ? SMALL : MEDIUM}>
...
const QuantityStepper = ({ id, quantity }) => {
  ...
  return (
    <div className="stepper-buttons">
      <Button.SECONDARY action={increment} customColor="lightGray">
        -
      </Button.SECONDARY>
      <span className="btn btn-ligth">{qty}</span>
      <Button.SECONDARY action={decrement} customColor="lightGray">
        +
      </Button.SECONDARY>
    </div>
  );
...
const CartTotal = ({ total_cart, discountedOffer }) => {
  ...
  return (
    <StyledCartRow>
      <ul>
        <li>
          <Text.BOLD>Subtotal: &nbsp; &nbsp;{total_cart.toFixed(2)}€</Text.BOLD>
        </li>
        <li>
          {isMinus || isSlice ? (
      ...
      <ul>
        <li>
          <Text.SUPERBOLD> Total: &nbsp; {total.toFixed(2)}€</Text.SUPERBOLD>
        </li>
      </ul>
    </StyledCartRow>
  );
};
export { CartTotal, TrashButton, CartButtons, QuantityInput, QuantityStepper };
